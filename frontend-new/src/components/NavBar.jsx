...
const NavBar = ({ isLoggedIn, onLoginClick, onSignupClick }) => {
    ...
    return (
        <div className='bg-blue-100/50 py-4 flex flex-row justify-between items-center'>
            ...
            <div className='flex flex-row gap-20 justify-center items-center bg-blue-200 rounded-4xl p-4 px-12 text-blue-950 font-bold'>
                {
                    NavBarMenu.map((option, index) => (
                        <Link 
                            key={index}
                            to={option.path}
                            className={`${location.pathname === option.path ? 'text-blue-600' : ''}`}
                        >
                            {option.title}
                        </Link>
                    ))
...
>
  Sign Up
      ...
      >
        <FaUser size={20} />
        {user !== 'Guest' ? 'Logged In' : 'Login'} {/* Show Login if not logged in, otherwise Logged In */}
      </button>
      <div className='bg-blue-600 rounded-full p-5 text-white'>
        <FaList size={20} />
      </div>
            </div>
        </div>
